def zero_matrix(x,y):
    return [[0 for i in range(y+1)] for j in range(x)]
...
if i==0 or j==0:
    ...
    if matrix==zero_matrix(i,j):
        print("Matrix given is a zero matrix and thus, always satisfies Ax=0")
    else:        
        for x in range(i):
        ...
        while check_echelon(matrix)==False:                                             #Converts to echelon form
        ...
        for row in range(-1,-len(matrix),-1):                                           #Echelon to RREF form
